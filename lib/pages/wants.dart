...
import 'package:budget_ui/pages/dialog/want_dialog.dart';
...
class _WantPageState extends State<WantPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: Text('Wants', style: TextStyle(color: black)),
        backgroundColor: Colors.grey[100],
        elevation: 0.0,
      ),
      body: ValueListenableBuilder<Box<transaksiWants>>(
          valueListenable: WantsBox.getTransactions().listenable(),
          builder: (context, box, _) {
            final transactions = box.values.toList().cast<transaksiWants>();
  ...
  Widget buildContent(List<transaksiWants> TransaksiWants) {
    final netExpense = TransaksiWants.fold<double>(
      0,
        ...
        children: [
          ...
          ListView.builder(
            ...
            itemBuilder: (BuildContext context, int index) {
              final transaction = TransaksiWants[index];
  ...
  Widget buildButtons(BuildContext context, transaksiWants transaction) => Row(
        children: [
          Expanded(
  ...
  Future addTransaction(String name, double amount, bool isExpense) async {
    final transaction = transaksiWants()
      ..name = name
      ..createdDate = DateTime.now()
      ..amount = amount
      ..isExpense = isExpense;

    final box = WantsBox.getTransactions();
    box.add(transaction);
  ...
  void deleteTransaction(transaksiWants transaction) {
    // final box = Boxes.getTransactions();
